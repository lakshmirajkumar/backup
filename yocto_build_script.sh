...
buildimage(){
	...
	then
		...
		sed '3iSCRIPT_DIR = '\"${SCRIPT_DIR}\"'' ${SCRIPT_DIR}/bbb/build/conf/bblayers.conf > tmp_file
		mv tmp_file ${SCRIPT_DIR}/bbb/build/conf/bblayers.conf
	fi
	...
	then
		...
		cd poky
		source ./oe-init-build-env ${SCRIPT_DIR}/rpi/build
		...
		sed '3iSCRIPT_DIR = '\"${SCRIPT_DIR}\"'' ${SCRIPT_DIR}/rpi/build/conf/bblayers.conf > tmp_file
		mv tmp_file ${SCRIPT_DIR}/rpi/build/conf/bblayers.conf
	fi
